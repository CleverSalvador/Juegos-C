...
#define dimensionAliado 2
#define dimensionEnemigo 2

using namespace miniwin;

void negro(int x, int y, int limite){
    while (x<limite){
      color_rgb(0, 0, 0);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
    }
}

void plomo(int x, int y, int limite){
    while (x<limite){
      color_rgb(50, 50, 50);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void azul(int x, int y, int limite){
    while (x<limite){
      color_rgb(60, 100, 250);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
    }
}

void verde(int x, int y, int limite){
    while (x<limite){
      color_rgb(0, 255, 200);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void celeste(int x, int y, int limite){
    while (x<limite){
      color_rgb(0, 200, 255);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void rojo(int x, int y, int limite){
    while (x<limite){
      color_rgb(255, 0, 0);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void naranja(int x, int y, int limite){
    while (x<limite){
      color_rgb(255, 155, 0);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void amarillo(int x, int y, int limite){
    while (x<limite){
      color_rgb(255, 255, 0);
      rectangulo_lleno(x,y,(x+dimensionAliado),(y+dimensionAliado));
      x=x+dimensionAliado;
...
void cero(int &x, int &limite, int z){
     x=z;
     limite=z;
}

void lineaSiguiente (int &y){
     y=y+dimensionAliado;
}

void longitud (int &limite, int longitud){
     limite=limite+(dimensionAliado*longitud);
}

void nuevoInicio (int &x, int &limite){
     x=limite;
}


void Player(int h, int k){
     int x=h;
     int limite=h;
     int y=k;
     int inicio=h;
...
void negroEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(0, 0, 0);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
    }
}

void azulEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(100, 100, 200);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
    }
}

void plomoEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
...
void verdeEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(0, 255, 200);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
...
void celesteEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(0, 200, 255);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
...
void rojoEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(255, 0, 0);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
...
void naranjaEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(255, 155, 0);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
...
void amarilloEnemigo(int i, int j, int limiteEnemigo){
    while (i<limiteEnemigo){
      color_rgb(255, 255, 0);
      rectangulo_lleno(i,j,(i+dimensionEnemigo),(j+dimensionEnemigo));
      i=i+dimensionEnemigo;
...
void ceroEnemigo(int &i, int &limiteEnemigo, int z){
     i=z;
     limiteEnemigo=z;
}

void lineaSiguienteEnemigo (int &j){
     j=j+dimensionEnemigo;
}

void longitudEnemigo (int &limiteEnemigo, int longitudEnemigo){
     limiteEnemigo=limiteEnemigo+(dimensionEnemigo*longitudEnemigo);
}

void nuevoInicioEnemigo (int &i, int &limiteEnemigo){
     i=limiteEnemigo;
}

void Enemy(int h, int k){
     int i=h;
     int limiteEnemigo=h;
     int j=k;
     int inicio=h;
    ...
    * FILA 1
    */
    longitudEnemigo(limiteEnemigo, 8);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
       
    longitudEnemigo(limiteEnemigo, 1);
    amarilloEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);

    longitudEnemigo(limiteEnemigo, 5);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    * FILA 2
    */
    ceroEnemigo(i, limiteEnemigo, inicio);
    lineaSiguienteEnemigo(j);

    longitudEnemigo(limiteEnemigo, 7);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
       
    longitudEnemigo(limiteEnemigo, 2);
    naranjaEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 4);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    longitudEnemigo(limiteEnemigo, 3);
    amarilloEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 2);
    plomoEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 1);
    verdeEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 3);
    celesteEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 6);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
    ...
    */
    ...
    longitudEnemigo(limiteEnemigo, 11);
    negroEnemigo(i,j,limiteEnemigo);
    nuevoInicioEnemigo(i, limiteEnemigo);
...
int main(){
    ...
    std::string ScoreText = "Puntuación = ";
    std::string BestScoreText = "Mejor puntuación = ";
    std::string GameOverText = "Game Over. Puntuación Final = ";
    std::string RestartText = "Reinicie con la tecla arriba";
